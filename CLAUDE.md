...
- **Shared control of ONE avatar is NOT co-op.** A mode where a single character is passed/hijacked between two people — P2 has no independent character of their own, only grab/assist/seize-control over the SAME avatar (Pizza Tower's "Swap Mode": follower can only Grab/Taunt/Dash-assist and take over the one Peppino) — fails this gate. Real 2-player co-op needs a second *controllable character on screen*. This is a plain "not a fit", NOT a 🟠 (🟠 presupposes a real second character whose role is merely episodic/secondary — it cannot rescue a mode that has no second character at all). Route to `reeval`, not `hard-block` — the criteria may evolve.
