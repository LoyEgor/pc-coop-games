...
6b. **`ownerConfirmed: true` is SACRED — never remove, never propose-removal, never re-classify endingType/genres on it.** An entry with `ownerConfirmed: true` was a decision the OWNER made by hand (typically a soft 🟠 finish like Icarus that you would otherwise read as "endless"). Skip such entries entirely in rule 7 (endless removal), in §9 (endingType), and in the consistency/reconcile passes — do not even log them to owner-review. You MAY still fix their media (broken youtube/image) and let the cron update price/rating, but the editorial verdict/endingType/finish-strength is locked. This is what stops the round-trip where you delete what the owner just re-added.
7. **Endless removal — fact-checker is now the enforcer** (this moved here from coop-hunter's old `revalidate_existing`, 2026-05-27). Two tiers (and NEVER touch `ownerConfirmed: true` entries — see 6b):
